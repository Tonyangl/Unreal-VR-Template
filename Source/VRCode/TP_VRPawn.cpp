...
void AVRPawn::BeginPlay()
{
	...
	if ( GEngine->XRSystem.IsValid()&& GEngine->XRSystem->GetHMDDevice())
	{
		...
		if ( ControlScheme == ETeleportControlScheme::Auto )
			ControlScheme =
				(DeviceName == TEXT("OculusRift") ? ETeleportControlScheme::StickOnly :
				(DeviceName == TEXT("PSVR") ? ETeleportControlScheme::ControllerRoll
																: ETeleportControlScheme::ButtonAndStick ) );

		if (DeviceName == TEXT("SteamVR") || DeviceName == TEXT("OculusRift"))
		{
