...
void AVRHand::RumbleController_Implementation( float intensity )
{
	...
	EControllerHand tempMotionSource = EControllerHand::Left;
	actionInfo.CallbackTarget = this;
	APlayerController *playerController = GetWorld()->GetFirstPlayerController();
	if (Hand==TEXT("Right"))
	{
		tempMotionSource = EControllerHand::Right;
	}
	else if(Hand == TEXT("Left"))
	{
		tempMotionSource = EControllerHand::Left;
	}
	else if (Hand == TEXT("Gun"))
	{
		tempMotionSource = EControllerHand::Gun;
	}
	playerController->PlayHapticEffect( HapticEffect, tempMotionSource, intensity, false );
}
...
bool AVRHand::TraceTeleportDestination( TArray<FVector> &TracePoints, FVector &NavMeshLocation, FVector &TraceLocation )
{
	...
	const UNavigationSystemV1* NavSys = GetDefault<UNavigationSystemV1>();
	const bool DidProjectToNav = NavSys->ProjectPointToNavigation( PointToProject, ProjectedHitLocation, Extents );
	if ( !DidProjectToNav )
		return false;
